...
- **Modules vs live:** reusable, environment-agnostic modules in `infra/modules/<name>`
  ...
  `iam-oidc`). Root configs in `infra/live/<env>/` (`dev`, `prod`) pass per-env inputs via
  tfvars / workspaces. Modules never hardcode env — env comes from inputs.
- **State:** remote in S3 with **DynamoDB locking**. One **state key per component per env**
  (e.g. `botp/dev/network`, `botp/dev/ecs`). No shared monolithic state.
...
Prefer `make` targets (`make validate`, `make scan`, `make test`) that wrap the above so CI
and local runs stay identical.
...
- Keep `docs/ARCHITECTURE.md`, `docs/SLO.md`, `docs/SECURITY.md`, and `docs/PLAN.md`
  consistent with the ADRs — especially the fixed decisions ADR-0004 (observability) and
  ADR-0005 (ledger datastore).
